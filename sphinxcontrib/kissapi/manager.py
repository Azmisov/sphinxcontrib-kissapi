...
class KissAPIDirective(SphinxDirective):
	...
	def run(self):
		...
		logger.info(f"kissapi Sphinx directive arguments: {self.arguments}")
		if self.output not in self.mgr.output:
			raise KeyError(f"{self.output} is not a valid output for this kiss-api Sphinx directive; an output must be specified in kissapi_config before use")
...
class RenderManager:
	...
	def __init__(self, app):
		...
		logger.info(f"Using kissapi_config: {self.conf}")
